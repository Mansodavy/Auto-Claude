...
// Add app.asar.unpacked to module resolution paths for production
// This allows packaged apps to find modules that were extracted from the asar
if (process.resourcesPath) {
  const unpackedModulesPath = pathJoin(process.resourcesPath, 'app.asar.unpacked', 'node_modules');
  if (!require('module').globalPaths.includes(unpackedModulesPath)) {
    require('module').globalPaths.push(unpackedModulesPath);
  }
}
